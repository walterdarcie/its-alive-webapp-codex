...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  return {
    title: `Seguidores — ${name}`,
    description: `Pessoas que seguem ${name} no it's alive.`
  };
}

export default async function ProfileFollowersPage({ params }: { params: Params }) {
  ...
  const [profilePayload, followsPayload] = await Promise.all([
    ...
    fetchJson<{ items: FollowListItem[] }>(
      `/api/profiles/${encodeURIComponent(params.userId)}/follows?type=followers`
    )
  ]);
